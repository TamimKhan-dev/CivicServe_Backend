...
const staffApplication = async (payload: IStaffApplicationPayload, userId: string) => {
    ...
    if (user?.status === "SUSPENDED") {
        throw new AppError(httpStatus.NOT_FOUND, "Your account has been Suspended!");
    };

    if (!user?.emailVerified) {
        throw new AppError(httpStatus.BAD_REQUEST, "Your email is not verified. verify to send Application!");
    };
    ...
    if (existingApplication) {
        throw new AppError(httpStatus.CONFLICT, "You already have a pending staff application!");
    };
